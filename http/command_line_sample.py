...
class LocalFabric(L.Fabric):
    buffer = ""

    def print(self, *args: any, **kwargs: any) -> None:
        if self.local_rank == 0:
            print(super, *args, **kwargs)
            for arg in args:
                self.buffer += arg
...
def run_query(prompt: str,
              checkpoint_dir: Path,
              tokenizer: Tokenizer, fabric: LocalFabric, model: GPT,
              top_k: int = 200,
...
if __name__ == '__main__':
    ...
    result = run_query(
        ...
        checkpoint_dir=checkpoint_dir,
        temperature=0.1,
        tokenizer=tokenizer, fabric=fabric, model=model
    )
