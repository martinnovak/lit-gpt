...
class LitGenerator:
    ...
    def decode(self, fabric: L.Fabric, tokenizer: Tokenizer, token_stream: Iterator[torch.Tensor]) -> int:
        tokens_generated = 0
    ...
    def setup(self,
        ...
        # system_prompt, stop_tokens = self.prompt_config(checkpoint_dir, tokenizer)

        # return tokenizer, fabric, model
    ...
    def query(self,
        ...
        """
        Runs the query and returns str object with result.
        Args:
            top_k: The number of top most probable tokens to consider in the sampling process.
            temperature: A value controlling the randomness of the sampling process. Higher values result in more random
                samples.
        """
        system_prompt, stop_tokens = self.prompt_config(self.checkpoint_dir, self.tokenizer)
        prompt = system_prompt.format(prompt=prompt)

        encoded_prompt = self.tokenizer.encode(prompt, device=self.fabric.device)
        max_returned_tokens = self.model.config.block_size
        y = self.generate(
            self.model,
            encoded_prompt,
            max_returned_tokens,
        ...
        try:
            ...
            # tokens_generated = decode(fabric, tokenizer, token_stream=y)
            tokens_generated = 0

            res = ""
            for token in y:
                res += self.tokenizer.decode(token)
                tokens_generated += 1

            t = time.perf_counter() - t0
            self.model.reset_cache()
            print(
                f"\nTime for inference: {t:.02f} sec total, {tokens_generated} tokens, {tokens_generated / t:.02f} tokens/sec", file=sys.stderr
            )
    ...
    def prompt_config(self, checkpoint_dir: Path, tokenizer: Tokenizer) -> Tuple[str, Tuple[List[int], ...]]:
...
if __name__ == '__main__':
    ...
    print(f"Query finished in : {time.time() - t0:.02f} seconds.")
    print(f"Having result: {result}")
    print("----------------------------------------------------------------")

    t1 = time.time()
    result = generator.query(
        prompt="What is capital of Canada",
        temperature=0.1,
    )
    print(f"Query finished in : {time.time() - t1:.02f} seconds.")
    print(f"Having result: {result}")
